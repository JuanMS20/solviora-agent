...
TIPS = [
    ...
    "solviora chat -t web,terminal enables only specific toolsets for a focused session.",
    "solviora chat -s github-pr-workflow preloads a skill at launch.",
    "solviora chat -q \"query\" runs a single non-interactive query and exits.",
    "solviora chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "solviora chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "solviora chat --source telegram tags the session for filtering in hermes sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "solviora doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hermes dump outputs a compact setup summary — great for bug reports.",
    "solviora config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "solviora config edit opens config.yaml in your default editor.",
    "solviora config check scans for missing or stale configuration options.",
    ...
    "solviora skills search react --source skills-sh searches the skills.sh public directory.",
    "solviora skills check scans installed hub skills for upstream updates.",
    "solviora skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "solviora skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    ...
    "solviora auth add lets you add multiple API keys for credential pool rotation.",
    "hermes completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "solviora logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "solviora backup creates a zip backup of your entire Solviora home directory.",
    ...
    "solviora update syncs new bundled skills to ALL profiles automatically.",
    "solviora gateway install sets up Solviora as a system service (systemd/launchd).",
    "solviora memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hermes webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: solviora tools disables unused tools, solviora skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "solviora models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "solviora skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: solviora cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Solviora runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "solviora gateway install sets it up as a system service that starts on boot.",
    ...
    "MCP servers auto-generate toolsets at runtime — solviora tools can toggle them per platform.",
    ...
    "solviora chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "solviora login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "solviora status --deep runs deeper diagnostic checks across all components.",
    ...
    "solviora claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via solviora tools.",
