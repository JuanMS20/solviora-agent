"""solviora claw — OpenClaw migration commands.

Usage:
    solviora claw migrate              # Preview then migrate (always shows preview first)
    solviora claw migrate --dry-run    # Preview only, no changes
    solviora claw migrate --yes        # Skip confirmation prompt
    solviora claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    solviora claw migrate --no-backup  # Skip pre-migration snapshot
    solviora claw cleanup              # Archive leftover OpenClaw directories
    solviora claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Solviora gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Solviora gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'solviora stop'.")
    print()
...
def claw_command(args):
    """Route solviora claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: solviora claw <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print("Run 'solviora claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: solviora claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: solviora claw migrate --yes")
            return
    ...
    # restorable with `solviora import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: solviora import {backup_archive.name}")
        except Exception as e:
            print()
    ...
    # 'solviora claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: solviora claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: solviora claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  solviora claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  solviora claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  solviora config set OPENROUTER_API_KEY sk-or-v1-...")
