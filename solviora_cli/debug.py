"""``solviora debug`` — debug tools for Solviora Agent.

Currently supports:
    solviora debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    ...
    ran ``solviora debug share`` repeatedly.

    Deletion is now driven by the gateway's cron ticker
    (``gateway/run.py::_start_cron_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``solviora debug share`` also
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``solviora debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``solviora debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `solviora debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``solviora debug share`` invocation added ~20 MB of resident Python
    ...
    ``solviora debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"solviora debug delete {url}"
...
def run_debug_share(args):
    ...
    print(f"To delete now:  solviora debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: solviora debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'solviora debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``solviora debug`` call.
    ...
    # best-effort — any failure is swallowed so ``solviora debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: solviora debug <command>")
        print()
        print("Commands:")
