"""solviora hooks — inspect and manage shell-script hooks.

Usage::

    solviora hooks list
    solviora hooks test <event> [--for-tool X] [--payload-file F]
    solviora hooks revoke <command>
    solviora hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``solviora hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: solviora hooks {list|test|revoke|doctor}")
        print("Run 'solviora hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        ...
        print("See `solviora hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `solviora hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `solviora hooks test` and `solviora hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `solviora hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `solviora hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `solviora hooks doctor`.")
