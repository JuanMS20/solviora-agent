...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`solviora ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching solviora: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`solviora ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
