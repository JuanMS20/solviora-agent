...
def auth_add_command(args) -> None:
    ...
    raise SystemExit(f"`solviora auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `solviora auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `solviora auth` is called bare."""
