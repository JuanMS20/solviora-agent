"""Shell completion script generation for Solviora CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_solviora_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Solviora Agent bash completion
# Add to ~/.bashrc:
#   eval "$(solviora completion bash)"
...
_solviora_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_solviora_profiles)" -- "$cur"))
        return
    fi
...
complete -F _solviora_completion solviora
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef solviora
# Solviora Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(solviora completion zsh)"
...
_solviora() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_solviora_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'solviora command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Solviora Agent fish completion",
        "# Add to your config:",
        "#   solviora completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __solviora_profiles",
        "    echo default",
        ...
        "complete -c solviora -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c solviora -f -s p -l profile"
        " -d 'Profile name' -xa '(__solviora_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__solviora_profiles)' -d 'Profile name'"
                )
