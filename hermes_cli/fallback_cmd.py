...
Subcommands:
  ...
  hermes fallback add      Pick provider + model via the same picker as `solviora model`,
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `solviora model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `solviora model`, then append the selection to the chain."""
    ...
    print("  `solviora model` — select the provider + model you want as a fallback.")
    print()
