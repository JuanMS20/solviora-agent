...
from solviora_cli.timeouts import (
...
def _routermint_headers() -> dict:
    ...
    from solviora_cli import __version__ as _SOLVIORA_VERSION

    return {
        "User-Agent": f"SolvioraAgent/{_SOLVIORA_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[solviora-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        self._cache_ttl = "5m"
        try:
            from solviora_cli.config import load_config as _load_pc_cfg
        ...
        from solviora_logging import setup_logging, setup_verbose_logging
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from solviora_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://solviora-agent.example.com",
                        "X-OpenRouter-Title": "Solviora Agent",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif base_url_host_matches(effective_base, "api.routermint.com"):
                    client_kwargs["default_headers"] = _routermint_headers()
                elif base_url_host_matches(effective_base, "api.githubcopilot.com"):
                    from solviora_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        try:
                            from solviora_cli.auth import PROVIDER_REGISTRY
                        ...
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `solviora model`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `solviora model` to "
                        "select a provider, or run `solviora setup` for first-time "
                        "configuration."
                    )
        ...
        try:
            from solviora_cli.config import load_config as _load_agent_config
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "solviora"
                        except Exception:
                            pass
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from solviora_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from solviora_cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Solviora Agent.  Choose a model with at least "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        ...
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from solviora_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        turn-scoped).
        """
        from solviora_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from solviora_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``solviora chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `solviora setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Solviora "
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        """
        from solviora_constants import resolve_env_float, _resolve_env

        cfg = get_provider_request_timeout(self.provider, self.model)
        if cfg is not None:
            return cfg
        return resolve_env_float("SOLVIORA_API_TIMEOUT", "HERMES_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        env_timeout = _resolve_env("SOLVIORA_API_CALL_STALE_TIMEOUT", "HERMES_API_CALL_STALE_TIMEOUT")
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from solviora_cli.models import _should_use_copilot_responses_api
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        from solviora_time import now as _hermes_now
        now = _hermes_now()
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from solviora_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from solviora_cli.auth import resolve_codex_runtime_credentials
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from solviora_cli.auth import resolve_nous_runtime_credentials
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from solviora_cli.copilot_auth import resolve_copilot_token
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # wins over the SOLVIORA_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else resolve_env_float("SOLVIORA_API_TIMEOUT", "HERMES_API_TIMEOUT", 1800.0)
            )
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            try:
                from solviora_cli.model_normalize import normalize_model_for_provider
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "solviora",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from solviora_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from solviora_cli.models import lmstudio_model_reasoning_options
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        if not pre_tool_block_checked:
            try:
                from solviora_cli.plugins import get_pre_tool_call_block_message
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=solviora-agent"]
    ...
    def run_conversation(
        ...
        from solviora_logging import set_session_context
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if (
                        ...
                        from solviora_constants import display_hermes_home as _dhh_fn
                        _dhh = _dhh_fn()
                        _body_text = ""
                        try:
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: solviora login --provider nous")
                    ...
                    if (
                        ...
                        from solviora_constants import display_hermes_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Solviora-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: solviora config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: solviora config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `solviora auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: solviora setup", force=True)
