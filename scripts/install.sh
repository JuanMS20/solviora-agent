...
# Configuration
REPO_URL_SSH="git@github.com:JuanMS20/solviora-agent.git"
REPO_URL_HTTPS="https://github.com/JuanMS20/solviora-agent.git"
SOLVIORA_HOME="${SOLVIORA_HOME:-${HERMES_HOME:-$HOME/.solviora}}"
HERMES_HOME="$SOLVIORA_HOME"
...
#   code at /usr/local/lib/solviora-agent, command at /usr/local/bin/solviora,
#   data still at /root/.solviora (SOLVIORA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.solviora/solviora-agent"
            echo "                   default (root, Linux): /usr/local/lib/solviora-agent"
            echo "  --solviora-home PATH  Data directory (default: ~/.solviora, or \$SOLVIORA_HOME)"
            echo "  --hermes-home PATH    Legacy alias for --solviora-home (accepts \$HERMES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Solviora installs the code under"
            echo "  /usr/local/lib/solviora-agent and links the command into"
            echo "  /usr/local/bin/solviora (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SOLVIORA_HOME"
            echo "  (default /root/.solviora).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SOLVIORA_HOME/solviora-agent are preserved in-place."
            exit 0
            ;;
...
# Decide where the repo checkout + venv live, and where the `solviora` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SOLVIORA_HOME/solviora-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SOLVIORA_HOME/solviora-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/solviora-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SOLVIORA_HOME/solviora-agent — then preserve it)
#
# Always no-op when the user set --dir or $SOLVIORA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SOLVIORA_HOME.
    if is_termux; then
        INSTALL_DIR="$SOLVIORA_HOME/solviora-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SOLVIORA_HOME/solviora-agent/.git" ]; then
            INSTALL_DIR="$SOLVIORA_HOME/solviora-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/solviora-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/solviora-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/solviora"
        log_info "  Data:    $SOLVIORA_HOME (unchanged)"
        return 0
    fi
...
get_command_path() {
    ...
    if [ -x "$link_dir/solviora" ]; then
        echo "$link_dir/solviora"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/JuanMS20/solviora-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$SOLVIORA_HOME/node/bin/node" ]; then
        export PATH="$SOLVIORA_HOME/node/bin:$PATH"
        local found_ver=$("$SOLVIORA_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Solviora-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.solviora/node/..."
    ...
    # Place into ~/.solviora/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$SOLVIORA_HOME/node"
    mkdir -p "$SOLVIORA_HOME"
    mv "$extracted_dir" "$SOLVIORA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$SOLVIORA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$SOLVIORA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$SOLVIORA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$SOLVIORA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$SOLVIORA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.solviora/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Solviora Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="solviora-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Solviora behaves unexpectedly."
                    else
...
log_info "Termux note: browser/WhatsApp tooling is not installed by default; see the Solviora guide for optional follow-up steps."
...
setup_path() {
    log_info "Setting up solviora command..."

    if [ "$USE_VENV" = true ]; then
        # Prefer solviora entry point, fall back to hermes for legacy compat
        if [ -x "$INSTALL_DIR/venv/bin/solviora" ]; then
            SOLVIORA_BIN="$INSTALL_DIR/venv/bin/solviora"
        elif [ -x "$INSTALL_DIR/venv/bin/hermes" ]; then
            SOLVIORA_BIN="$INSTALL_DIR/venv/bin/hermes"
        else
            SOLVIORA_BIN=""
        fi
    else
        SOLVIORA_BIN="$(which solviora 2>/dev/null || which hermes 2>/dev/null || echo "")"
        if [ -z "$SOLVIORA_BIN" ]; then
            log_warn "solviora not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$SOLVIORA_BIN" ]; then
        log_warn "Entry point not found at $SOLVIORA_BIN"
    ...
    # Create user-facing shims for the solviora command.
    mkdir -p "$command_link_dir"
    ln -sf "$SOLVIORA_BIN" "$command_link_dir/solviora"
    # Legacy compatibility alias: `hermes` still works.
    ln -sf "$SOLVIORA_BIN" "$command_link_dir/hermes"
    log_success "Symlinked solviora → $command_link_display_dir/solviora"
    log_info "Legacy alias: hermes → $command_link_display_dir/hermes (temporary compat)"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "solviora command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v solviora' \
                >/dev/null 2>&1; then
        ...
        log_info "solviora not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Solviora Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    # Export for current session so solviora works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.solviora directory structure (config at top level, code in subdir)
    ...
    if [ ! -f "$SOLVIORA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            ...
            log_success "Created ~/.solviora/.env from template"
        else
            touch "$SOLVIORA_HOME/.env"
            log_success "Created ~/.solviora/.env"
        fi
    else
        log_info "~/.solviora/.env already exists, keeping it"
    fi
    ...
    if [ ! -f "$SOLVIORA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            ...
            log_success "Created ~/.solviora/config.yaml from template"
        fi
    else
        log_info "~/.solviora/config.yaml already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.solviora/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.solviora/"

    # Seed bundled skills into ~/.solviora/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.solviora/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.solviora/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SOLVIORA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$SOLVIORA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.solviora/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Solviora itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (solviora --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'solviora setup' after install."
        return 0
    fi
    ...
    # Run solviora setup using the venv Python directly (no activation needed).
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Solviora to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SOLVIORA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ...
                $SOLVIORA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'solviora whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'solviora gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SOLVIORA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SOLVIORA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: solviora gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: solviora gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SOLVIORA_CMD gateway > "$SOLVIORA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
        ...
        log_info "Skipped. Start the gateway later with: solviora gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SOLVIORA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SOLVIORA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SOLVIORA_HOME/cron/, sessions/, logs/"
    ...
    echo -e "   ${GREEN}solviora gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}solviora update${NC}     Update to latest version"
    echo ""
