"""Tests for the stale-dashboard handling run at the end of ``solviora update``.

``solviora update`` detects ``hermes dashboard`` processes left over from the
...
class TestKillStaleDashboardPosix:
    ...
    def test_permission_error_is_reported_not_raised(self, capsys):
        ...
        must not abort solviora update — it's reported as a failure and we
        move on."""
        def fake_kill(pid, sig):
...
class TestWindowsWmicEncoding:
    ...
    `solviora update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_returns_none_stdout_does_not_crash(self, monkeypatch):
        ...
        ``None.split('\\n')`` and aborting `solviora update` (#17049)."""
        monkeypatch.setattr(sys, "platform", "win32")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
