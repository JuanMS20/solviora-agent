...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Solviora starts and finds ``~/.openclaw/``.

    Points users at ``solviora claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``solviora claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Solviora, run "
        "`solviora claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `solviora claw cleanup` (renames it to ~/.openclaw.pre-migration — "
