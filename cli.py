#!/usr/bin/env python3
"""
Solviora Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for Solviora Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills solviora-dev,github-auth
...
from solviora_cli.banner import _format_context_length, format_banner_version_label

_COMMAND_SPINNER_FRAMES = ("⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏")


# Load .env from ~/.solviora/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from solviora_constants import get_hermes_home, display_hermes_home
from solviora_cli.browser_connect import (
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.solviora/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from solviora_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.solviora/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``solviora chat --ignore-user-config``),
    the user config at ``~/.solviora/config.yaml`` is skipped entirely and only the
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Solviora, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Solviora - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and SolvioraCLI.__init__ checks "default" first.
    ...
    from solviora_cli.config import _expand_env_vars
...
# Initialize centralized logging early — agent.log + errors.log in ~/.solviora/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from solviora_logging import setup_logging
...
try:
    from solviora_cli.config import print_config_warnings
...
try:
    from solviora_cli.skin_engine import init_skin_from_config
...
from solviora_cli.banner import build_welcome_banner
from solviora_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from solviora_cli.callbacks import prompt_for_secret
...
def _run_cleanup():
    ...
    try:
        from solviora_cli.plugins import invoke_hook as _invoke_hook
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run solviora -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"solviora-{short_id}"
    branch_name = f"solviora/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    if session_db is None:
        return
    try:
        from solviora_cli.config import load_config as _load_full_config
        from solviora_constants import get_hermes_home as _get_hermes_home
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``solviora/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("solviora-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``solviora/solviora-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``solviora -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("solviora/solviora-") or b.startswith("pr-"))
    ]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from solviora_cli.skin_engine import get_active_skin
...
from solviora_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``SolvioraCLI._busy_command()``.
        """
        yield self

# ASCII Art - SOLVIORA AGENT logo (full width, single line - requires ~95 char terminal)
SOLVIORA_AGENT_LOGO = """[bold #FFD700]███████╗ ██████╗ ██╗     ██╗   ██╗██╗ ██████╗ ██████╗  █████╗         █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██╔════╝██╔═══██╗██║     ██║   ██║██║██╔═══██╗██╔══██╗██╔══██╗       ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████╗██║   ██║██║     ██║   ██║██║██║   ██║██████╔╝███████║       ███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]╚════██║██║   ██║██║     ╚██╗ ██╔╝██║██║   ██║██╔══██╗██╔══██║       ██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]███████║╚██████╔╝███████╗ ╚████╔╝ ██║╚██████╔╝██║  ██║██║  ██║       ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# Backward-compatibility alias — skins/skills that import this from cli keep working
HERMES_AGENT_LOGO = SOLVIORA_AGENT_LOGO

# ASCII Art - Solviora emblem (compact, fits in left panel)
SOLVIORA_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
HERMES_CADUCEUS = SOLVIORA_CADUCEUS
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "SOLVIORA AGENT - AI Agent Framework"
        tiny_line = "SOLVIORA"
    else:
        agent_name = _skin.get_branding("agent_name", "Solviora Agent") if _skin else "Solviora Agent"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Solviora[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from solviora_cli.plugins import get_plugin_manager
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.solviora/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.solviora/config.yaml on first use)
...
class SolvioraCLI:
    """
    Interactive CLI for Solviora Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from solviora_cli.runtime_provider import _auto_detect_local_model
        ...
        try:
            from solviora_state import SessionDB
        ...
        # checkpoint repos under ~/.solviora/checkpoints/.  Opt-in via
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Solviora'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from solviora_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from solviora_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from solviora_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from solviora_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from solviora_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "Solviora")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from solviora_cli.runtime_provider import (
        ...
        if runtime is None and _primary_exc is not None:
            from solviora_cli.auth import AuthError
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: solviora setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: solviora setup")
            return False
        ...
        # without `solviora model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from solviora_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from solviora_cli.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (solviora sessions list).{_RST}")
                return False
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]Solviora needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        from solviora_cli.model_switch import is_nous_hermes_non_agentic
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with Solviora Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Solviora: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Solviora: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.solviora/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from solviora_cli.clipboard import save_clipboard_image
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: solviora --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Solviora config/state.
        ...
        """
        from solviora_cli.backup import (
            create_quick_snapshot, list_quick_snapshots,
            restore_quick_snapshot, prune_quick_snapshots,
        )
        from solviora_constants import display_hermes_home
    ...
    def _handle_paste_command(self):
        ...
        from solviora_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run solviora chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'solviora setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from solviora_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from solviora_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from solviora_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Solviora!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        from io import StringIO
        from solviora_cli.tools_config import tools_disable_enable_command
        ...
        from solviora_cli.tools_config import _get_platform_tools
        from solviora_cli.config import load_config
    ...
    def _handle_profile_command(self):
        ...
        from solviora_constants import display_hermes_home
        from solviora_cli.profiles import get_active_profile_name
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from solviora_cli.main import _relative_time
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `solviora sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from solviora_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `solviora sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.solviora/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``solviora --resume <id>``
        ...
        try:
            ...
            return
        path = saved_dir / f"solviora_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: solviora --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from solviora_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from solviora_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            if not model_list:
                try:
                    from solviora_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from solviora_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from solviora_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        from solviora_cli.providers import get_label
        ...
        custom_provs = None
        try:
            from solviora_cli.config import get_compatible_custom_providers, load_config
            cfg = load_config()
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from solviora_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        try:
            from solviora_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from solviora_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        from solviora_cli.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        from solviora_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in ("quit", "exit", "q"):
            ...
            if self._app:
                ...
                try:
                    from solviora_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from solviora_cli.skin_engine import get_active_skin
            ...
            from solviora_cli.config import reload_env
            count = reload_env()
            ...
            # Check for plugin-registered slash commands
                ...
                from solviora_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from solviora_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from solviora_cli.goals import GoalManager
            from solviora_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Solviora keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from solviora_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Solviora is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Solviora is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Solviora is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from solviora_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from solviora_cli.debug import run_debug_share
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from solviora_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.solviora/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "solviora_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "solviora_voice",
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  solviora --resume {self.session_id}")
            if session_title:
                print(f"  solviora -c \"{session_title}\"")
            print()
            ...
            try:
                from solviora_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from solviora_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from solviora_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Solviora Agent! Describe what you need, or try:")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Solviora Agent! Describe what you need, or try:"
            _welcome_color = "#FFF8DC"
        self._console_print(f"[{_welcome_color}]{_welcome_text}[/]")
        try:
            _dim = _welcome_skin.get_color("banner_dim", "#B8860B") if _welcome_skin else "#B8860B"
        except Exception:
            _dim = "#B8860B"
        self._console_print(f"[{_dim}]  → \"search for the latest news on any topic\"")
        self._console_print(f"[{_dim}]  → \"read main.py and summarize the architecture\"")
        self._console_print(f"[{_dim}]  → \"write a Python script that fetches weather data\"")
        self._console_print(f"[{_dim}]  → /help for all commands[/]")
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from solviora_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from solviora_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Solviora Agent")
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Solviora needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Solviora needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Solviora needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = SolvioraCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: solviora setup"
            )
            _run_cleanup()
...
# Backward-compatibility alias — old imports of ``HermesCLI`` keep working
HermesCLI = SolvioraCLI
...
def main(
    ...
    """
    Solviora Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Solviora Gateway (messaging platforms)...")
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Solviora" box, no tool-gen
